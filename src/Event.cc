...
namespace irene {
  ...
  std::vector<std::vector<double> > Event::FillHitVector(std::string det) const
  {
    ...
    for (size_t ihit=0; ihit<evthits.size(); ++ihit)
    {
      std::pair<TLorentzVector,double> evhit = evthits[ihit];
      std::vector<double> nhit;
      for (size_t i=0; i<3; ++i)
        nhit.push_back(evhit.first[i]);
      nhit.push_back(evhit.second);
      hits.push_back(nhit);
    }
    return hits;
  }
